...
No.
This is not a limitation of Spectre Browser but of all Chromium-based projects in general, as general public is not allowed to use Google's APIs for free unless when using Chromium.

Additionally, these features would not be privacy-friendly.

## Does Spectre Browser require root?
No.

## Is Spectre Browser de-googled?

Yes, but if you were to find connections to cloud-based services please report them via the issue tracker.

Spectre Browser uses [Internal firewall patch](https://github.com/uazo/Spectre Browser/blob/master/build/patches/Internal-firewall.patch) to disable unwanted connections and [ungoogled-chromium: no special hosts/domains](https://github.com/uazo/Spectre Browser/blob/master/build/patches/ungoogled-chromium-no-special-hosts-domains.patch) for the removal of all code that uses pointers to Google servers.

Please note that the patch is called ungoogled-chromium but has nothing to do with the project of the same name.

## Does Spectre Browser support DRM media?
...
## Is Spectre Browser on Play Store?
No, but there are plans to activate it.

## Is Spectre Browser on F-Droid?
It is not on the official F-Droid repository.

You can use F-Droid client to install and receive updates via [the official Spectre Browser F-Droid repository](https://www.Spectre Browser.org/fdroid/repo).

## Using Spectre Browser will favour the monopoly of the Chromium/Blink engine, why do you develop and maintain Spectre Browser?
...
## Does Spectre Browser support extensions in Android?
...
when websites use the [Push API](https://w3c.github.io/push-api/); this will not work in Spectre Browser because cloud integrations are disabled (GCM in this case).

## Can TWA/PWAs be installed?

No. TWAs will not work because they are generated server-side on googleapis.com (which is not allowed in Spectre Browser).

What is supported is home shortcuts that launch the browser without its interface.

## Does Spectre Browser support the Android autofill framework?

Yes, the native Android autofill can be used.

## Does Spectre Browser support casting media content?

No, this would require Play Store binary blobs.

## Can you add this search engine as default?
No. Spectre Browser does not make any choice related to default search engines, user will have to decide during the first launch.
You are still free to enter any search engine you prefer via the settings ui.

## Some websites show ads, how can I fix this?
Spectre Browser uses public lists for adblock that you see in the settings. Please check if the issue is due to the rules (and if so, report the problem in its correct repo) before requesting a check.

## This website is not performing well with Spectre Browser, how can I fix this?
Spectre Browser comes with [JavaScript JIT](https://hacks.mozilla.org/2017/02/a-crash-course-in-just-in-time-jit-compilers/) disabled by default.
...
ADB in Spectre Browser is enhanced with support for cname uncloaking and blocking even in workers (a unique feature of Spectre Browser). Support for "acceptable ads" has been completely removed.
...
For this reason, in Spectre Browser (and its predecessor Spectre Browser) it was chosen to disable JIT by default, penalising execution but offering a smaller attack surface.
...
Spectre Browser blocks the ability to open apps from the web by default.
